...
def pre_power(power: pd.DataFrame) -> pd.DataFrame:
    ...
    p1_df = p1_df.groupby(["id", pd.Grouper(key="dated", freq="W")]).mean().reset_index()
    p2_df = p2_df.groupby(["id", pd.Grouper(key="dated", freq="W")]).mean().reset_index()
    p3_df = p3_df.groupby(["id", pd.Grouper(key="dated", freq="W")]).mean().reset_index()
